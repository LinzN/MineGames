...
public class SettingsManager {
    ...
    public void setup(Plugin p) {
        SettingsManager.p = p;
        if (p.getConfig().getInt("config-version") == MineGames.config_version) {
            MineGames.config_todate = true;
        } else {
    ...
    public boolean moveFile(File ff) {
        MineGames.$("Moving outdated config file. " + f.getName());
        String name = ff.getName();
        File ff2 = new File(MineGames.getPluginDataFolder(), getNextName(name, 0));
        return ff.renameTo(ff2);
    }

    public String getNextName(String name, int n) {
        File ff = new File(MineGames.getPluginDataFolder(), name + ".old" + n);
        if (!ff.exists()) {
            return ff.getName();
        } else {
