                ...
                if (gameid != -1) {
                    ...
                    if (game.getMode() == GameMode.INGAME) {
                        ...
                        if (!openedChest.containsKey(e.getClickedBlock())) {
                            ...
                            MineGames.debug("ChestReplaceEvent: " + blk.getX() + "," + blk.getY() + "," + blk.getZ());
                            ...
                            for (Inventory inv : invs) {
                                if (SettingsManager.getInstance().getConfig().getBoolean("clear", false)) {
                                    MineGames.debug("Clearing initial content");
//        							inv.setContents(new ItemStack[inv.getContents().length]);
                                ...
                                for (ItemStack i : ChestRatioStorage.getInstance().getItems(level)) {
                                    ...
                                    int attempt = 0;
                                    if (inv.firstEmpty() < 0) {
                                        MineGames.info("Chest at " + blk.getX() + "," + blk.getY() + "," + blk.getZ() + " is full: cannot add items");
                                        break;
                                    }
                                    while ((inv.getItem(l) != null) && (attempt < 20)) { // warning!  Chest may be full!
                                        MineGames.debug("Look again, slot " + l + " is already occupied!");
                                        l = rand.nextInt(26);
                                        attempt++;
                                    }
                                    ...
                                    try {
                                        if (attempt < 20) {
                                            ...
                                            MineGames.info("Chest at " + blk.getX() + "," + blk.getY() + "," + blk.getZ() + " is too full: cannot add items");
                                            break;
                                        }
                                    } catch (Exception e2) {
                                        MineGames.error("Problem putting item " + i.getType().name() + " into slot " + l);
                                        MineGames.error("Java says: " + e2.getMessage());
                                    }
                                }
                            }
                            if (SettingsManager.getInstance().getConfig().getBoolean("debug", false)) {
                                for (ItemStack is : oldinv) {
                                    if (is != null) {
                                        try {
                                            MineGames.debug("Saved item " + is.getType().name() + " DV " + is.getDurability() + " qty " + is.getAmount());
                                        } catch (Exception e2) {
                                            MineGames.debug("Error listing saved item");
                                        }
                                    }
                                }
                            }
