...
public class FileCache {
    ...
    public static void loadPage(String pagename, boolean template) {
        ...
        Scanner scan = null;
        File f = new File(MineGames.getPluginDataFolder() + ((template) ? "/www/template.html" : "/www/pages/" + pagename + ".html"));

        try {
            scan = new Scanner(f);
        } catch (Exception e) {
            System.out.println("Survival Games webstats - Could not load page: " + pagename + "    " + f.getAbsolutePath());
        }
        String data = "";
